<script lang="ts">
    ...
    import SocialsIcon from "$lib/assets/img/icons/socials.png";
...
<nav class="bg-black fixed flex flex-row items-center gap-4 justify-between w-screen min-h-15 border-(--border-color) border-b-4 p-2">
    ...
    <div class="not-2xl:min-w-60 not-2xl:h-screen not-2xl:fixed not-2xl:p-4 {!navigationPanelIsOpen && "not-2xl:transform-[translate(-100%)]"} flex gap-2 flex-col 2xl:flex-row bg-black left-0 top-0 border-(--border-color) not-2xl:border-r-4 transition">
        ...
        <div class="2xl:ml-auto flex 2xl:flex-row flex-col gap-2">
            ...
            <NavigationLinkElement href="/socials" imgSrc={SocialsIcon}>Socials</NavigationLinkElement>
...
<div class="max-w-400 pt-25 m-auto p-2">
    {@render children()}
</div>