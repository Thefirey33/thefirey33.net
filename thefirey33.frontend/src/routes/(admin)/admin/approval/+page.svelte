<script lang="ts">
    ...
    async function changeState(index: number, uuid: string, state: boolean) {
        ...
        if (result) {
            ...
            setToast("Failed to change state!")
...
<button class="btn mb-5 flex group gap-4" onclick={async () => getApprovalData()}>
    <img alt="Refresh" class="group-hover:invert transition" src={RefreshIcon}/>
    Refresh
</button>
...
{#if approvalData.length <= 0 || apiLoading}
    ...
    {:else}
        <p class="text-white text-xl">No pending approval requests yet.</p>
    {/if}
{:else}
    <table>
        <thead>
        ...
        {#each approvalData as approval, index (index)}
            <tr>
                ...
                <td>
                    ...
                    <button onclick={() => changeState(index, approval.uuid, true)}
                            class="btn text-base mb-3 {approval.approved ? "pointer-events-none opacity-50" : ""}">
                        Allow
                    </button>
                    <button onclick={() => changeState(index, approval.uuid, false)}
